...
import { createPantrySurprise, recommendRecipes } from '@/src/lib/recommendations';
...
export default function SurpriseScreen() {
  ...
  const { state, hydrated, markRecipeChosen, saveGeneratedRecipe, setRecipeFeedback, addRecipeMissingToShoppingList } = useApp();
  const autoGenerated = useRef(false);
  const [recipe, setRecipe] = useState<Recipe>(() => localFallbackRecipe(state, 'quick', 'dinner'));
  ...
  useEffect(() => {
    if (!hydrated || !state.pantry.length || autoGenerated.current) return;
    autoGenerated.current = true;
    void generate('quick', 'dinner', false);
  }, [hydrated, state.pantry.length]);
...
function localFallbackRecipe(state: ReturnType<typeof useApp>['state'], time: TimeBucket, meal: MealCategory) {
  return recommendRecipes(RECIPES, state, time, meal)[0]?.recipe
    ?? createPantrySurprise(state, time, meal);
}
